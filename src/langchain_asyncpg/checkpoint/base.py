...
class BaseAsyncpgSaver(BaseCheckpointSaver[str]):
    ...
    def _search_where(
        ...
        if config:
            ...
            wheres.append(f"thread_id = ${len(param_values)} ")
            checkpoint_ns = config["configurable"].get("checkpoint_ns")
            if checkpoint_ns is not None:
                param_values.append(checkpoint_ns)
                wheres.append(f"checkpoint_ns = ${len(param_values)}")

            if checkpoint_id := get_checkpoint_id(config):
                param_values.append(checkpoint_id)
                wheres.append(f"checkpoint_id = ${len(param_values)} ")

        # construct predicate for metadata filter
        if filter:
            param_values.append(filter)
            wheres.append(f"metadata @> ${len(param_values)}::jsonb")
